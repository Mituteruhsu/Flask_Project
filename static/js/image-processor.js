...
class ImageProcessor {
    constructor() {
        ...
        // 預處理階段預覽畫布
        this.canvasExposure = document.getElementById('canvasExposure');
        this.canvasClahe = document.getElementById('canvasClahe');
    ...
    preprocessMat(src, { updatePreview = true } = {}) {
        ...
        // 曝光補償（提升亮度/對比）
        const exposureComp = 1.6; // 1.0 = 不變，>1 變亮
        cv.convertScaleAbs(gray, gray, exposureComp, 0);

        // 曝光後將接近黑的像素壓到全黑
        const blackClamp = 20; // 0~255，越大黑色範圍越廣
        cv.threshold(gray, gray, blackClamp, 255, cv.THRESH_TOZERO);

        // 曝光補償後預覽
        if (updatePreview && this.canvasExposure) {
            this.canvasExposure.width = gray.cols;
            this.canvasExposure.height = gray.rows;
            cv.imshow(this.canvasExposure, gray);
        }

        // 曝光後使用 CLAHE（Contrast Limited AHE）
        if (cv.createCLAHE) {
            const clahe = cv.createCLAHE(1.0, new cv.Size(16, 16));
            clahe.apply(gray, binary);
            clahe.delete();
        ...
        // CLAHE 後預覽
        if (updatePreview && this.canvasClahe) {
            this.canvasClahe.width = binary.cols;
            this.canvasClahe.height = binary.rows;
            cv.imshow(this.canvasClahe, binary);
        }
        gray.delete();
        return binary;
    }
